...
class Blog_Post_Notice_Editor {
	...
	public function post_notice_display() {
		require_once(
			plugin_dir_path( __FILE__ ) . 'views/blog-post-notice-editor.php'
		);
	}
	...
	public function save_post_notice( $post_id ) {
		...
		$post_notice = $_POST[ 'blog-post-notice-editor' ];
		$post_notice = stripslashes( strip_tags( $post_notice ) );

		update_post_meta( $post_id, 'blog-post-notice', $post_notice );
	...
	public function user_can_save( $post_id ) {
		...
		$is_valid_nonce =
			( isset( $_POST['blog-post-notice-nonce'] ) ) &&
			wp_verify_nonce(
				$_POST[ 'blog-post-notice-nonce' ],
				'blog-post-notice-save'
			);
